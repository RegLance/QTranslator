...
void SelectionHook::SendCopyKey(CopyKeyType type)
{
    ...
    // Build input array — symmetric: every press has a matching release
    std::vector<INPUT> inputs;
    INPUT input = {};

    // if Alt is pressing, release it first, re-press it after
    if (isAltPressing)
    {
    ...
    // Press Ctrl (left Ctrl, universally recognized)
    if (!isCtrlPressing)
    {
        input.type = INPUT_KEYBOARD;
    ...
    // Re-press Alt/Shift if we released them
    if (isAltPressing)
    {
        input.type = INPUT_KEYBOARD;
        input.ki.wVk = VK_MENU;
