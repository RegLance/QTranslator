...
• 截图识字（OCR）：框选屏幕区域，识别文字后填入翻译窗口，进行翻译
• 润色功能：对文本进行润色改进，可在设置中开启差异标记
• 总结功能：对长文本进行智能总结
• 划词写作：翻译并直接替换原文，支持保留原文选项
• 翻译历史：自动保存历史记录，方便查阅和管理
• 单词收藏：输入框标星收藏原文与译文；右键托盘菜单「单词收藏」集中浏览、搜索、删除、导出
• 词汇短文：在单词收藏窗口内按复习次数选取收藏词，选择体裁后由模型生成约 160 词内的短文，便于巩固词汇
• 多主题：支持深色、浅色及多种彩色主题，也可自定义主题
• 语种检测：语种联网（百度 / Google / Bing）或本地检测，联网检查失败则退回本地检查方式
• 朗读：翻译窗口、历史记录、单词收藏等界面可朗读
...
5. 选中翻译（快捷键）
   • 在当前应用先选中需要翻译的文字，然后按下快捷键（默认 Ctrl+Shift+T）
   • 程序会读取当前选中内容并在鼠标附近打开翻译窗口，取词优先级与自动划词一致
   • 若未检测到选中内容，可先使用 Ctrl+C 复制后再试或确认输入焦点在正文区域
   • 快捷键可在设置 → 「选中翻译」中修改

6. 截图识字（OCR）
   • 默认 Ctrl+Shift+O，或托盘菜单「截图识字」
   • 全屏变暗后拖选矩形区域，Esc 取消；识别完成后自动打开翻译窗口并填入原文，一般会继续自动翻译
   • 设置 →「截图识字 (OCR)」可选识别语种：中文/英文（默认），或韩文（需额外下载模型）

7. 单词收藏与词汇短文
   • 收藏：翻译窗口译文区右下角星标；取消收藏可仅按原文操作
   • 打开：托盘菜单「单词收藏」，列表中可查看详情、搜索、删除选中、导出 JSON 或清空（清空请谨慎）
   • 词汇短文：窗口下部选择体裁后点「生成短文」，按复习次数从高到低取词（最多 50 条，不足则用已有全部）生成短文；可「停止」生成，生成结果可朗读
...
• 翻译方向：中文→英文，其它→中文等
• 单词翻译会显示详细释义、音标和例句
• 词汇短文与翻译、润色、总结一样依赖已配置的 API；OCR 多语种需额外模型文件（见设置页说明）
• 翻译结果仅供参考，请核实重要内容
