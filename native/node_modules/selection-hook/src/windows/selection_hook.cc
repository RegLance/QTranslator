...
bool SelectionHook::GetTextViaClipboard(HWND hwnd, TextSelectionInfo &selectionInfo)
{
    ...
    if (!isInDelayReadList)
    {
        ...
        // Atomic guard: check again right before SendInput — if clipboard
        // already changed (user pressed Ctrl+C in the microseconds between
        // our check and now), read directly instead of sending keys.
        if (GetClipboardSequenceNumber() != clipboard_sequence)
        {
            if (ReadClipboard(selectionInfo.text) && !selectionInfo.text.empty())
            {
    ...
    // Atomic guard: check right before SendInput
    if (GetClipboardSequenceNumber() != clipboard_sequence)
    {
        if (ReadClipboard(selectionInfo.text) && !selectionInfo.text.empty())
        {
...
void SelectionHook::SendCopyKey(CopyKeyType type)
{
    ...
    // Press the Ctrl key (use right Ctrl to avoid interfering with left Ctrl)
    if (!isCtrlPressing)
    {
        input.type = INPUT_KEYBOARD;
    ...
    // Release the key (Insert or C) — BUT skip if the user is physically
    // holding this key right now. Otherwise our key-up would break the
    // user's ongoing Ctrl+C and cause a stray character to be typed.
    bool keyStillPressed = (GetAsyncKeyState(keyCode) & 0x8000) != 0;
    if (!keyStillPressed)
    {
        input.type = INPUT_KEYBOARD;
        input.ki.wVk = keyCode;
    ...
    // else: user is holding this key — their own release will handle it.

    // Release the Ctrl key — same check: skip if user is still holding it.
    if (!isCtrlPressing)
    {
        bool ctrlStillPressed = (GetAsyncKeyState(VK_CONTROL) & 0x8000) != 0;
        if (!ctrlStillPressed)
        {
            input.type = INPUT_KEYBOARD;
            input.ki.wVk = VK_RCONTROL;
...
bool SelectionHook::ShouldKeyInterruptViaClipboard()
{
    ...
    // User is pressing Ctrl + (C/X/V) — they are copying/cutting/pasting.
    // Don't interfere by sending our own simulated keystrokes.
    bool isCtrlPressing = (GetAsyncKeyState(VK_CONTROL) & 0x8000) != 0;
    if (!isCtrlPressing)
    {
        return false;
    ...
    bool isXPressing = (GetAsyncKeyState('X') & 0x8000) != 0;
    bool isVPressing = (GetAsyncKeyState('V') & 0x8000) != 0;
    return isCPressing || isXPressing || isVPressing;
}
