...
class VocabularyWindow(QWidget):
    ...
    def __init__(self) -> None:
        ...
        # 缓存上一次容器尺寸，避免 setGeometry 导致 Resize 死循环抖动
    ...
    def _setup_ui(self) -> None:
        ...
        self._list.setStyleSheet(
            ...
            QListWidget {{ background: {theme['input_bg']}; color: {theme['text_primary']};
                border: 1px solid {theme['border_color']}; border-radius: 6px; outline: none; }}
        ...
        self._trans_plain.setStyleSheet(
            ...
            QPlainTextEdit:focus {{ outline: none; }}
            {get_scrollbar_style(theme)}
        """
        )
        ...
        self._speak_word_btn.setStyleSheet(
            ...
            QPushButton#vocSpeakWordBtn:focus {{
                outline: none;
            }}
        """
        )
        ...
        self._bb_speak_btn.setStyleSheet(
            ...
            QPushButton#vocBbSpeakBtn:focus {{
                outline: none;
            }}
        """
        )
    ...
    def _update_trans_output_layout(self) -> None:
        try:
            ...
            if w <= 0 or h <= 0:
                return
            if (w, h) == self._last_trans_size:
                return
            self._last_trans_size = (w, h)
    ...
    def _update_bb_output_layout(self) -> None:
        try:
            ...
            if w <= 0 or h <= 0:
                return
            if (w, h) == self._last_bb_output_size:
                return
            self._last_bb_output_size = (w, h)
    ...
    def _speak_bb_output(self) -> None:
        ...
        if (
            ...
            if self._tts_speak_bb_prep.is_preparing():
                # 同一按钮：toggle 停止
    ...
    def _speak_word(self) -> None:
        ...
        if (
            ...
            if self._tts_speak_word_prep.is_preparing():
                # 同一按钮：toggle 停止
                self._stop_tts_playback()
                return
            # 其他按钮/窗口：停止后切换
    ...
    def _export_json(self) -> None:
        path, _ = QFileDialog.getSaveFileName(
            self, "导出单词收藏", str(Path.home() / "Downloads" / "qtranslator_vocabulary.json"), "JSON (*.json)"
        )
        if not path:
            return
        try:
